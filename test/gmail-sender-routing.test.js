...
test('both independently credentialed active senders are eligible for dental', () => assert.deepEqual(configuredSenders(env).map(s => [s.id,s.sendEligible]), [['primary',true],['b',true],['scalelabaiteam',false],['deniels',false],['deniels_tryscalelabai',false]]));
