...
test('demo booking-link promotion stops ordinary cadence in the same and later runs', () => {
  ...
  assert.ok(intent.indexOf("lead.stage = 'Promoted'") < agent.indexOf('const followUps = selectFollowUps(all, ownershipActivities)'), 'same-pass selector sees promoted state');
});
