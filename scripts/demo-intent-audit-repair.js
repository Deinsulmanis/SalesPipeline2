...
async function main() {
  ...
  if (repairLeadId && !confirmed) throw new Error('repair requires --confirm-no-send');
  ...
  const snapshot = await api.spreadsheets.values.batchGet({
    ...
    ranges: ['ColdEmail!A:X','Leads!A:W','ColdCallActivity!A:J','DemoPlays!A:F','Suppression!A:A'],
  });
  ...
  for (const lead of cold) {
    ...
    if (!bookingLinkEventFor(lead, mine) && !replied && !meeting
  ...
  const report = {
    mode: repairLeadId ? 'targeted-repair' : 'read-only-audit',
